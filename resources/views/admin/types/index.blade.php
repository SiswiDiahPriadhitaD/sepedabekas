            ...
            <tbody>
              @foreach ($data as $item)    
                <tr>
                  ...
                  <td>
                    <form action="/admin/type/{{ $item->id }}" method="POST" onsubmit="return confirm('apakah anda yakin akan mengahpus data {{ $item->type }}')">
                      @csrf
                      @method('DELETE')
                      <a href="/admin/type/{{ $item->id }}/edit" class="btn btn-sm btn-info text-white">Ubah</a>
